...
data "aws_ami" "ecs_ami" {
  ...
  owners = [data.terraform_remote_state.vpc.outputs.vpc_account_id, "895523100917"] # MOJ
}
...
locals {
  ...
  sg_bastion_in_id             = data.terraform_remote_state.security-groups.outputs.sg_ssh_bastion_in_id
  environment_identifier       = var.environment_identifier
  short_environment_identifier = var.short_environment_identifier
  lb_security_groups           = [data.terraform_remote_state.security-groups.outputs.sg_monitoring_elb]
  ...
  eng_vpc_cidr                 = data.terraform_remote_state.vpc.outputs.eng_vpc_cidr
  port                         = 9200
  protocol                     = "HTTP"
  ...
  efs_security_groups = [
    data.terraform_remote_state.security-groups.outputs.sg_mon_efs,
  ]

  instance_security_groups = [
    data.terraform_remote_state.security-groups.outputs.sg_ssh_bastion_in_id,
    data.terraform_remote_state.security-groups.outputs.sg_mon_efs,
    data.terraform_remote_state.security-groups.outputs.sg_monitoring,
    data.terraform_remote_state.security-groups.outputs.sg_elasticsearch,
  ]
