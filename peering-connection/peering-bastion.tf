...
resource "aws_vpc_peering_connection" "peering-bastion-vpc" {
  peer_owner_id = "${data.terraform_remote_state.bastion_remote_vpc.vpc.account_id}"
  peer_vpc_id   = "${data.terraform_remote_state.bastion_remote_vpc.vpc.id}"
...
module "route-to-bastion-vpc-private-cidr-az1" {
  ...
  destination_cidr_block = "${data.terraform_remote_state.bastion_remote_vpc.vpc.private_cidr.az1}"
  vpc_peer_id            = "${aws_vpc_peering_connection.peering-bastion-vpc.id}"
  create                 = 1
}
...
module "route-to-bastion-vpc-private-cidr-az2" {
  ...
  destination_cidr_block = "${data.terraform_remote_state.bastion_remote_vpc.vpc.private_cidr.az2}"
  vpc_peer_id            = "${aws_vpc_peering_connection.peering-bastion-vpc.id}"
  create                 = 1
}
...
module "route-to-bastion-vpc-private-cidr-az3" {
  ...
  destination_cidr_block = "${data.terraform_remote_state.bastion_remote_vpc.vpc.private_cidr.az3}"
  vpc_peer_id            = "${aws_vpc_peering_connection.peering-bastion-vpc.id}"
  create                 = 1
}
