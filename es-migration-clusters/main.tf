...
data "terraform_remote_state" "bastion_remote_vpc" {
  ...
  config = {
    bucket   = data.terraform_remote_state.vpc.outputs.bastion_remote_state_bucket_name
    key      = "bastion-vpc/terraform.tfstate"
    region   = var.region
    role_arn = data.terraform_remote_state.vpc.outputs.bastion_role_arn
  }
}
...
module "create_elastic2_efs_backup_share" {
  ...
  domain          = data.terraform_remote_state.vpc.outputs.private_zone_name
...
module "create_elastic2_cluster" {
  ...
  bastion_origin_cidr          = data.terraform_remote_state.bastion_remote_vpc.outputs.bastion_vpc_cidr
  bastion_origin_sgs           = local.bastion_origin_sgs

  private_zone_name  = data.terraform_remote_state.vpc.outputs.private_zone_name
