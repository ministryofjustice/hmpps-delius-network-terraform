...
resource "aws_security_group" "elasticsearch_client_sg" {
  name        = "${var.app_name}-${var.environment_identifier}-elasticsearch-sg"
  description = "security group for ${var.environment_identifier}-elasticsearch"
  vpc_id      = "${var.vpc_id}"

  tags = "${merge(var.tags, map("Name", "${var.app_name}-${var.environment_identifier}-elasticsearch-sg"))}"
}
...
resource "aws_security_group_rule" "elasticsearch_client_sg_es_http_in" {
  ...
  to_port = 9200
  type = "ingress"
  description = "${var.app_name}-${var.environment_identifier}-es-http-traffic"
