...
resource "aws_security_group" "ce_sg" {
  ...
  tags        = "${merge(var.tags, map("Name", "${local.name_prefix}-testing-out", "Type", "Private"))}"
  egress {
    from_port   = 0
    to_port     = 0
    protocol    = "-1"
...
resource "aws_batch_compute_environment" "batch_ce" {
  compute_environment_name = "${local.name_prefix}-testing-ce"

  compute_resources {
    instance_role = "${aws_iam_instance_profile.ecs_instance_profile.arn}"
    ec2_key_pair = "${data.terraform_remote_state.vpc.ssh_deployer_key}"
